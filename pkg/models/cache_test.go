...
func TestGetCacheEngine(t *testing.T) {
	cases := []struct {
		...
		},
		{
			cacheEngineString:   "",
			expectedCacheEngine: "",
			expectedErr:         errors.New("Unkown cache engine type . Supported engines are: MEMORY or REDIS"),
		},
		{
			cacheEngineString:   "DUMMY",
			expectedCacheEngine: "",
			expectedErr:         errors.New("Unkown cache engine type DUMMY. Supported engines are: MEMORY or REDIS"),
		},
	}
